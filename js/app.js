...
doc.setFontSize(14);
doc.text("Strategic B&B Investment Report",20,y);
...
doc.setFontSize(10);
doc.text("Generated by RendimentoBB Strategic Engine",20,y);
...
// ================= ADVANCED EXECUTIVE PDF =================

window.generateExecutivePDF = function(){
  ...
  if(!window.lastAnalysisData){
    ...
    alert(
      window.currentLang==="it"
      ? "Genera prima l'analisi investimento."
      : "Run the investment analysis first."
    );
  ...
  doc.text(
    "Property Price: " + formatCurrency(data.price),
    20,
    y
  );
  ...
  doc.text(
    "Loan Amount: " + formatCurrency(data.loan),
    20,
    y
  );
  ...
  doc.text("Estimated Annual Revenue",20,y);
  y += 10;
