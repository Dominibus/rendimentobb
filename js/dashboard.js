...
window.updateBookingTouristTax = function(){
  ...
  const guests = Math.max(0, Math.floor(Number(
    document.getElementById("booking-guests")?.value || 0
  )));
  const taxableGuestsField = document.getElementById("booking-taxable-guests");

  if(taxableGuestsField && taxableGuestsField.dataset.manual !== "true"){
    taxableGuestsField.value = String(guests);
  }

  const taxableGuests = Math.min(
    guests,
    Math.max(0, Math.floor(Number(taxableGuestsField?.value || 0)))
  );
  if(taxableGuestsField && Number(taxableGuestsField.value) > guests){
    taxableGuestsField.value = String(guests);
  }
  ...
  const taxableNights = maxNights > 0
    ? Math.min(stayNights, maxNights)
    : stayNights;
  const amount = taxableGuests * taxableNights * Number(config.ratePerGuestNight || 0);
  const symbol = window.getTouristTaxCurrencySymbol(config.currency);
  const amountField = document.getElementById("booking-tourist-tax-amount");
  const ruleField = document.getElementById("booking-tourist-tax-rule");

  if(amountField) amountField.textContent = `${symbol}${amount.toFixed(2)}`;
  if(ruleField){
    ruleField.textContent = window.t(
      `${symbol}${Number(config.ratePerGuestNight || 0).toFixed(2)} × ${taxableGuests} ospiti × ${taxableNights} notti tassabili · età minima ${Number(config.minimumTaxableAge || 0)}`,
      `${symbol}${Number(config.ratePerGuestNight || 0).toFixed(2)} × ${taxableGuests} guests × ${taxableNights} taxable nights · minimum age ${Number(config.minimumTaxableAge || 0)}`
    );
  }
...
window.openBookingModal = async function(){
    ...
    if(taxableGuests){
      taxableGuests.value = "";
      taxableGuests.dataset.manual = "false";
    }
    const taxStatus = document.getElementById("booking-tourist-tax-status");
    if(taxStatus) taxStatus.value = "pending";
...
window.currentSelectedBooking = booking;
    ...
    const taxableGuestsField = document.getElementById("booking-taxable-guests");
    const touristTaxStatus = document.getElementById("booking-tourist-tax-status");
    const touristTaxMethod = document.getElementById("booking-tourist-tax-method");

    if(taxableGuestsField){
      taxableGuestsField.value = savedTouristTax.taxableGuests ?? Number(booking.guests || 0);
      taxableGuestsField.dataset.manual = "true";
    }
    if(touristTaxStatus){
      touristTaxStatus.value = savedTouristTax.status || "pending";
    }
    if(touristTaxMethod){
...
if(!window.currentPropertyId){
  ...
  window.updateBookingTouristTax();
  const touristTaxConfig = window.currentPropertyTouristTaxConfig;
  const touristTaxBox = document.getElementById("booking-tourist-tax-box");
  const touristTax = touristTaxConfig?.enabled ? {
    enabled: true,
    taxableGuests: Math.max(0, Math.floor(Number(
      document.getElementById("booking-taxable-guests")?.value || 0
    ))),
    taxableNights: Math.max(0, Number(touristTaxBox?.dataset.taxableNights || 0)),
    ratePerGuestNight: Number(touristTaxConfig.ratePerGuestNight || 0),
    amount: Math.max(0, Number(touristTaxBox?.dataset.amount || 0)),
    currency: touristTaxConfig.currency || "EUR",
    minimumTaxableAge: Number(touristTaxConfig.minimumTaxableAge || 0),
    status: document.getElementById("booking-tourist-tax-status")?.value || "pending",
    paymentMethod: document.getElementById("booking-tourist-tax-method")?.value || "cash",
    collectionTime: touristTaxConfig.collectionTime || "checkin"
  } : {
    enabled: false,
    amount: 0
  };
...
${b.touristTax?.enabled ? (() => {
  const taxStatusLabels = {
    ...
    exempt: window.t("Esente", "Exempt"),
    platform: window.t("Riscossa dal portale", "Collected by platform")
  };
  const taxStatus = b.touristTax.status || "pending";
  const taxSymbol = window.getTouristTaxCurrencySymbol(b.touristTax.currency);
  const isPending = taxStatus === "pending";
  return `
    <div style="margin-top:12px;padding:10px 12px;border-radius:12px;background:${isPending ? "#fff7ed" : "#f0fdf4"};border:1px solid ${isPending ? "#fed7aa" : "#bbf7d0"};display:flex;justify-content:space-between;align-items:center;gap:10px;">
      <span style="font-size:12px;font-weight:700;color:#475569;">🏛️ ${window.t("Tassa di soggiorno", "Tourist tax")}</span>
      <strong style="font-size:12px;color:${isPending ? "#c2410c" : "#15803d"};">
        ${taxSymbol}${Number(b.touristTax.amount || 0).toFixed(2)} · ${taxStatusLabels[taxStatus] || taxStatusLabels.pending}
      </strong>
    </div>
  `;
})() : ""}
